...
public class Fly extends TestNGCitrusSpringSupport {
    ...
    public void successfulFlyActiveWings(@Optional @CitrusResource TestCaseRunner runner) {
        ...
        validateResponse(runner, "I am flying :)"); //В документации смайлика нет. Валидацию без него надо проходить скорее всего
    ...
    public void successfulFlyFixedWings(@Optional @CitrusResource TestCaseRunner runner) {
        ...
        flyDuck(runner, "${duckId}");
        validateResponse(runner, "I can not fly :C"); //Аналогично
    }
    ...
    public void successfulFlyUndefinedWings(@Optional @CitrusResource TestCaseRunner runner) {
        createDuck(runner, "yellow", 0.03, "rubber", "quack", "UNDEFINED");//В документации про UNDEFINED информации нет, но есть в ошибке Bad Request
        flyDuck(runner, "${duckId}");
        validateResponse(runner, "Wings are not detected :("); //Аналогично
    }
}
